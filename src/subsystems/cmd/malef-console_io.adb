...
package body Malef.Console_IO is
   ...
   begin
      if Format /= Last_Format then
         Flush;
         Dummy := SetConsoleTextAttribute (Standard_Output, Format);
         Last_Format := Format;
      end if;
   end Set_Attribute;
   ...
   is
      ...
      -- TODO: Use SetConsoleTitleW for Wide_Character support
      function SetConsoleTitle (
         lpConsoleTitle : in chars_ptr)
         return BOOL with
         Import        => True,
         Convention    => Stdcall,
         External_Name => "SetConsoleTitleA";
